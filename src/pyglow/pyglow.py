...
class Point(object):
    ...
    def set_date(self,dn):
        
        self.dn = dn 
        
        self.location_time.dn = self.dn
        
        self.indice = None
        self.indice = Indice(self.dn)
        gc.collect()
    ...
    def set_latlon(self,lat,lon):
        ...
        self.location_time.lat = self.lat
        self.location_time.lon = self.lon
        
    
    def set_height(self,valor):
        
        self.alt = valor
        self.location_time.alt= self.alt     
    ...
    def run(self):
        ...
        if self.alt is not None :
            ...
            elif isinstance(self.alt,list) or isinstance(self.alt,numpy.ndarray):
                
                for height in self.alt:
                    self.location_time.alt = height
                    self.run_iri(self.version)
                    self.ne_array.append(self.ne)
            else:
                AttributeError("Proporcione un valor correcto de alturas.")
    ...
    def run_iri(
        ...
        if self.user_ind:
            f107 = self.f107
            f107a = self.f107a
            if numpy.isnan(f107) or numpy.isnan(f107a):
                raise ValueError(
    ...
    def run_airglow(self):
        ...
        # appropriate models:
        if numpy.isnan(self.ne):
            self.run_iri()
        if numpy.isnan(self.nn['O2']):
            self.run_msis()
        ...
        K1_6300 = 3.23e-12*numpy.exp(3.72/ti - 1.87/ti**2)
        K2_6300 = 2.78e-13*numpy.exp(2.07/ti - 0.61/ti**2)
        K3_6300 = 2.0e-11*numpy.exp(111.8/Tn)
        K4_6300 = 2.9e-11*numpy.exp(67.5/Tn)
    ...
    def get_table(self):
        #define table for load
        GEOPHYSICAL_INDICES_FNAME = ""
        return numpy.load(GEOPHYSICAL_INDICES_FNAME)
...
def _igrf_tracefield_hemis(dn, lat, lon, alt, target_ht, step):
    ...
    lla = numpy.array([lat, lon, alt*1e3])
    ...
    while (lla[2] > target_ht):
        ...
        lla_field = numpy.vstack([lla_field, lla])
        i = i + 1
    ...
    while (abs(lla[2]-target_ht) > TOLERANCE):
        ...
        step = -numpy.sign(step)*abs(lla[2]-target_ht)
        ...
        ecef_new = ecef + coord.ven2ecef(
            lla,
            numpy.array([-D/A, E/A, N/A]) * step/(-D/A),
        )
